...
function mgr_group_check ($a_uid, $a_flag, $a_app) {
  ...
  if (strlen($mgr_groups[$a_app])==0) {return;}
...
function mail_group_check ($a_dn, $a_flag, $a_ml) {
  ...
  if (strlen($a_flag)==0) {
    ...
    if ($aCnt==0) {
      ...
      $tmp_err = error_reporting($old_err);
      if ($err != 0) {
        $_SESSION['in_msg'] .= "$warn ldap error adding $a_ml to $a_dn: "
                 . "$err - $err_msg.$ef";
      } else {
        $_SESSION['in_msg'] .= "$ok Mail List $a_ml added to $a_dn.$ef";
      }
    }
  }
}
...
if ( !(isset($_SESSION['in_msg']) ) {$_SESSION['in_msg'] = " ";}
...
if (strlen($btn_add)>0) {
  ...
  if (strlen($in_new_cn)==0) {
    ...
    if ($uid_cnt>0) {
      ...
      while (strlen($a_ml)>0) {
        $_SESSION['in_msg'] .="$ok adding mailDistributionID = $a_ml$ef";
