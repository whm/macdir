...
function add_emaillist_authorization ($a_uid) {
    ...
    if ($app_cnt == 0) {
        ...
        if ($app_cnt == 0) {
            ...
            // Okay, they are a nobody.  Add them to the email-lists
            // application group
            ...
            if ($err == 0) {
                ...
                $_SESSION['in_msg']
                    .= "$warn ldap error adding $a_uid to email-lists:"
                    . "$err - $err_msg.$ef";
            }
        }
    }
    ...
    if ($addr_type == 2) {
        ...
        if (strlen($a_flag)==0) {
            ...
            if ($aCnt==0) {
                ...
                if ($err != 0) {
                    $_SESSION['in_msg']
                        .= "$warn ldap error adding $a_ml to $aDN: "
                        . "$err - $err_msg.$ef";
                } else {
                    $_SESSION['in_msg']
                        .= "$ok Mail List $a_ml added to $aDN.$ef";
                    $update_cnt++;
                }
            }
...
if (strlen($in_button_add)>0) {
    ...
    if (ldap_maillist_admin() == 0) {
        ...
        if ($ml_cnt>0) {
            ...
            $_SESSION['in_msg']
                .= "$warn Mail List is already in use by $tmp.$ef";
            ...
            $_SESSION['in_msg']
                .= "$ok adding objectClass = prideemaillist$ef";
            ...
            if (@ldap_add($ds, $this_dn, $ldap_entry)) {
                ...
                $_SESSION['in_msg']
                    .= "$warn Error: $ldapErr, $ldapMsg<br>$ef";
            }
...
} elseif (strlen($in_button_update)>0) {
    ...
    if ($ret_cnt>0) {
        if (ldap_maillist_authorization($info[0]["manageruid"]) < 1) {
            $_SESSION['in_msg']
                .= $warn."$in_localmailbox is inaccessible".$ef;
            $ret_cnt = 0;
            unset($info);
        }
    }
...
} elseif (strlen($in_button_delete)>0) {
